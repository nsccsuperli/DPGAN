...
for epoch in range(epoch, num_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_G = loss_GAN + lambda_1 * loss_1 +lambda_con_edg *loss_edg
        loss_G.backward()
        optimizer_G.step()
        best_loss = loss_G
        sys.stdout.write("\r[Epoch %d/%d: batch %d/%d] [DLoss: %.3f, GLoss: %.3f, AdvLoss: %.3f, EdgeLoss: %.3f, ConLoss: %.3f]"
                         % (
                             epoch, num_epochs, i, len(dataloader),
                             loss_D.item(), loss_G.item(), loss_GAN.item(),loss_edg.item(), loss_con.item()
                         )
                         )
        ## Print log
        if not i%50:
